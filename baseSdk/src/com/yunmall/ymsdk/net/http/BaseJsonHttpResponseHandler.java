...
import com.yunmall.ymsdk.net.http.thread.CacheThreadPool;
...
public abstract class BaseJsonHttpResponseHandler<JSON_TYPE> extends TextHttpResponseHandler {
    ...
    @Override
    public void onCacheLoad(final String responseString, final long time) {
        Runnable parser = new Runnable() {
            @Override
            public void run() {
                try {
                    final JSON_TYPE jsonResponse = parseResponse(responseString, false);
                    addMarker("parsed cache: %s", responseString);
                    postRunnable(new Runnable() {
                        @Override
                        public void run() {
                            onCacheLoad(jsonResponse, time);
                        }
                    });
                } catch (final Throwable t) {
                    addMarker("parsed cache exception: %s", t);
                    postRunnable(new Runnable() {
                        @Override
                        public void run() {
                            onFailure(0, null, t, responseString, null);
                        }
                    });
                }
            }
...
//            new Thread(parser).start();
            CacheThreadPool.getInstance().submit(parser);
        } else { // In synchronous mode everything should be run on one thread
            parser.run();
        }
    }

    public void onCacheLoad(JSON_TYPE responseJsonType, long time) {
    }
    ...
    public final void onSuccess(final int statusCode, final Header[] headers, final String responseString) {
	    ...
	    Runnable parser = new Runnable() {
                ...
                public void run() {
                    try {
                        final JSON_TYPE jsonResponse = parseResponse(responseString, false);
                        addMarker("parsed http response: %s", responseString);
                        finishLog("finish");
                        postRunnable(new Runnable() {
                            @Override
                            public void run() {
                        ...
                        addMarker("parsed http response exception: %s", t);
                        finishLog("finish");
                        t.printStackTrace();
                        postRunnable(new Runnable() {
                            @Override
	    ...
	    Runnable parser = new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        YmLog.d(LOG_TAG, "onFailure parseResponse thrown an problem", t);
                        finishLog("finish");
                        postRunnable(new Runnable() {
                            @Override
                            public void run() {
