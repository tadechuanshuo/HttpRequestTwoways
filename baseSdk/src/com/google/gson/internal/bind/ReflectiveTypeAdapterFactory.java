...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
	...
	private ReflectiveTypeAdapterFactory.BoundField createBoundField(
		...
		return new ReflectiveTypeAdapterFactory.BoundField(name, serialize,
				deserialize) {
			final TypeAdapter<?> typeAdapter = getFieldAdapter(context, field,
					fieldType);
			...
			void read(JsonReader reader, Object value) throws IOException,
				...
				if (typeAdapter instanceof EnumTypeAdapter) {
					fieldValue = ((EnumTypeAdapter) typeAdapter).read(reader,
							fieldType);
				} else {
	...
	private TypeAdapter<?> getFieldAdapter(Gson gson, Field field,
		...
		if (annotation != null) {
			TypeAdapter<?> adapter = getTypeAdapter(constructorConstructor,
					gson, fieldType, annotation);
			if (adapter != null)
				return adapter;
		}
